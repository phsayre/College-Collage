	...
	public static void AStar(){
		...
		while(true){
			...
			if(current.i-1>=0){
				...
				checkAndUpdateCost(current, t, current.fCost+V_H_COST);
				...
				if(current.j+1<grid[0].length){
					t = grid[current.i-1][current.j+1];
					checkAndUpdateCost(current, t, current.fCost+DIAGONAL_COST);
				}
			}
